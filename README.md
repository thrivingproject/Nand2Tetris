# Nand2Tetris

Self-study of Elements of computing systems. Projects completed by following textbook suggestions. Some comments and API method signatures are copied directly from the textbook as cited below.

1. [Elementary Logic Gates](./proj1/)
1. [Digital Circuits](./proj2/)
1. [Memory Chips](./proj3/)
1. [Assembly](./proj4/)
1. [CPU / Computer Architecture](./proj5/)
1. [Assembler](./proj6/)
1. [Virtual Machine Translator](./proj7/)
1. [Compiler](./proj10/)

## Works Cited

Nisan, Noam, and Shimon Schocken. The Elements of Computing Systems, Second Edition : Building a Modern Computer from First Principles, MIT Press, 2021. ProQuest Ebook Central, [https://ebookcentral.proquest.com/lib/harvard-ebooks/detail.action?docID=6630880](https://ebookcentral.proquest.com/lib/harvard-ebooks/detail.action?docID=6630880).
